...
class MainWindow(Adw.ApplicationWindow):
    def __init__(self, *args, **kwargs):
        ...
        menu.append("About Muse", "win.about") # Changed to win.
        ...
        self.title_widget = Adw.WindowTitle(title="Muse")
        ...
        self.search_bar.bind_property("search-mode-enabled", self.search_btn, "active", 
        ...
        from ui.pages.playlist import PlaylistPage # Import here to avoid circular dep if any
        ...
        self.set_content(self.root_content_view)
        
        # Initialize Pages (Must be before breakpoint)
        self.init_pages()
        
        # Responsive Breakpoint
        breakpoint = Adw.Breakpoint.new(Adw.BreakpointCondition.new_length(Adw.BreakpointConditionLengthType.MAX_WIDTH, 550, Adw.LengthUnit.PX))
    ...
    def on_playlist_header_title_changed(self, page, title):
        if hasattr(self, 'title_widget'):
            self.title_widget.set_title(title if title else "Muse")
    ...
    def update_back_button_visibility(self, *args):
        ...
        if nav:
            ...
            if visible_page and nav.get_previous_page(visible_page):
                ...
                if hasattr(self, 'title_widget'):
                    self.title_widget.set_title("Muse")
        else:
            self.back_btn.set_visible(False)
    ...
    def show_about(self, action, param):
        ...
        about.set_application_name("Muse")
        about.set_developer_name("POCOGuy")
        about.set_version("alpha 202602192035")
    ...
    def init_pages(self):
        ...
        page_home = self.view_stack.add_named(create_tab_nav(home_page, "Home", "user-home-symbolic", "home"), "home")
        page_home.set_title("Home")
        page_home.set_icon_name("user-home-symbolic")

        page_lib = self.view_stack.add_named(create_tab_nav(self.library_page, "Library", "media-optical-symbolic", "library"), "library")
        page_lib.set_title("Library")
        page_lib.set_icon_name("media-optical-symbolic")

        page_lib.set_icon_name("media-optical-symbolic")

        page_search = self.view_stack.add_named(create_tab_nav(search_page, "Explore", "compass2-symbolic", "search"), "search")
    ...
    def open_playlist(self, playlist_id, initial_data=None):
        ...
        playlist_page.connect('header-title-changed', self.on_playlist_header_title_changed)
    ...
    def open_artist(self, channel_id, initial_name=None):
        ...
        nav_page = Adw.NavigationPage(child=artist_page, title=initial_name if initial_name else "Artist")
        ...
        artist_page.connect('header-title-changed', self.on_playlist_header_title_changed) # Reuse same handler
